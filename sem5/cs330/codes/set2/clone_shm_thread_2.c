...
int add (void *arg)
{
	...
	printf("Child saying: my sum is %llu\n", child_sum);
        doneChild=1;
        while (!doneParent) sleep(0);	// Compiler won't register-allocated doneParent across system calls
...
int main (void)
{
	...
	// wait for child
	while(!doneChild) sleep(0);	// Compiler won't register-allocated doneParent across system calls
                                        // because it assumes the worst-case scenario: doneParent may be updated
                                        // by the system call handler.
