...
void hp_inter_lower(const list<LINE>& Lin, list<LINE>& Lout)
{
 Lout.clear();
 if (Lin.size() < 1) return;
 ...
 list<POINT> output = CONVEX_HULL(Hcand);
 list<POINT> lower = LOWER_PART(output);
...
int main()
{
 ...
 geo_scene res1= gw.new_scene(hp_inter_lower, sc1, string("Halfplane intersection"), H1,H1);
