...
static leda_mutex mutex1;
static leda_mutex mutex2;
...
{ 
  ...
  swap(*k,*j);
  if (j > l+1) new_job(l,j-1,lock);
  if (r > j+1) new_job(j+1,r,lock);
...
{ 
  ...
  mutex2.lock();
  cout << string("thread: id = %2d  (%08x) time = %.2f sec", 
                          id, thread_self(), elapsed_time(t)) << endl;
  mutex2.unlock();
