...
#define LEDA_ROOT_INCL_ID 600148
...
public:
  ...
  void   set_params(void* wp, char* cp, int w = 0, 
                    double v1 =0, double v2=0, double v3=0, double v4=0)
  { win_ptr=(window*)wp; char_ptr=cp; i=w; d[0]=v1; d[1]=v2; d[2]=v3; d[3]=v4; }
...
double xmax_no_grid() const { return min_xcoord+xdots*one_over_scaling; }
double ymin_no_grid() const { return min_ycoord; }
double ymax_no_grid() const { return min_ycoord+(ydots-panel_height)*one_over_scaling; }
...
void  pixrect_to_ppm(char* pmap, ostream& ppmout);

void  screenshot_ps(const char* fname, bool full_color=true);
void  screenshot_wmf(const char* fname, bool full_color=true);

void  screenshot_ppm(ostream& ps_stream);
void  screenshot_ppm(const char* fname);
...
void draw_color_button(int xt, int yt, int w, color col, int pressed, 
                                                         int enabled=1);
...
int  scrollbar_width() { return (scroll_bar) ? scroll_bar->window_width : 0; } 
